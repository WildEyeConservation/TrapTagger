...
function buildBrowserUpload(divID) {
    ...
    if (document.getElementById('addImagesTGCode')!=null) {
        ...
        $("#newSurveyCalCodeInput").change(function() {
...
function buildFolders(path, divID, folderType='site') {
    ...
    if (path) {
        ...
        folders.forEach((folder, index) => {
            ...
            folderDiv.className = "folderbtn " + folderType + "-folder";
    ...
    if (browserChecked) {
        ...
        if ((tgCode!='')&&(camCode!='')&&(pathDisplay.options.length>0)) {
            if (tgCode.endsWith('.*') || tgCode.endsWith('.+') || tgCode.endsWith('.*[0-9]+') || tgCode.endsWith('.+[0-9]+' )) {
                ...
                indices = {}
                for (let i=2;i<pathDisplay.options.length;i++) {
                    if (pathMatchesCalibration(pathDisplay.options[i].text)) continue
...
function buildAddIms() {
    ...
    calOptionsSection.setAttribute('id', 'calibrationOptionsSectionES')
    calOptionsSection.hidden = true
    addFilesDiv.appendChild(calOptionsSection)

    // Calibration Code

    h5 = document.createElement('h5')
    h5.setAttribute('style','margin-bottom: 2px')
    h5.innerHTML = 'Calibration'
    calOptionsSection.appendChild(h5)

    div = document.createElement('div')
    div.setAttribute('style','font-size: 80%; margin-bottom: 2px')
    div.innerHTML = '<i>Do you have calibration images in this survey?</i>'
    calOptionsSection.appendChild(div)
    ...
    input.setAttribute('type','radio')
    input.classList.add('custom-control-input')
    input.setAttribute('id','noCalImagesES')
    input.setAttribute('name','calSelectionES')
    input.setAttribute('value','customEx')
    input.checked = true
    input.setAttribute('onchange','updateCalDiv()')
    radioDiv.appendChild(input)

    var label = document.createElement('label')
    label.classList.add('custom-control-label')
    label.setAttribute('for','noCalImagesES')
    label.innerHTML = 'No'
    radioDiv.appendChild(label)
    ...
    input.setAttribute('type','radio')
    input.classList.add('custom-control-input')
    input.setAttribute('id','yesCalImagesES')
    input.setAttribute('name','calSelectionES')
    input.setAttribute('value','customEx')
    input.checked = false
    input.setAttribute('onchange','updateCalDiv()')
    ...
    var label = document.createElement('label')
    label.classList.add('custom-control-label')
    label.setAttribute('for','yesCalImagesES')
    label.innerHTML = 'Yes'
    radioDiv.appendChild(label)


    var calDetailsDiv = document.createElement('div')
    calDetailsDiv.setAttribute('id', 'calDetailsDivES')
    calDetailsDiv.hidden = true
    calOptionsSection.appendChild(calDetailsDiv)

    var calOptionDesc = document.createElement('div')
    calOptionDesc.setAttribute('style', 'font-size: 80%; margin-bottom: 2px')
    calOptionDesc.setAttribute('id', 'calOptionDescES')
    calOptionDesc.innerHTML = '<i>The identifier used to designate a calibration folder in your folder structure. Eg. "extra" matches a folder named "extra", or "Cal" matches "Cal", "Cal1", "Cal2", etc. Becomes a regular expression search query if the advanced option is selected.</i>'
    calDetailsDiv.appendChild(calOptionDesc)

    var calIdDiv = document.createElement('div')
    calIdDiv.setAttribute('id', 'calIdDivES')
    calIdDiv.hidden = true
    calOptionsSection.appendChild(calIdDiv)

    var calRow = document.createElement('div')
    calRow.classList.add('row')
    calIdDiv.appendChild(calRow)

    var calCol1 = document.createElement('div')
    calCol1.classList.add('col-lg-4')
    calRow.appendChild(calCol1)

    var calInput = document.createElement('input')
    calInput.setAttribute('type', 'text')
    calInput.classList.add('form-control')
    calInput.required = true
    calInput.setAttribute('id', 'addImagesCalCodeInput')
    calCol1.appendChild(calInput)

    var calCol2 = document.createElement('div')
    calCol2.classList.add('col-lg-4')
    calRow.appendChild(calCol2)

    var calCheckDiv = document.createElement('div')
    calCheckDiv.setAttribute('class', 'custom-control custom-checkbox')
    calCol2.appendChild(calCheckDiv)

    var calAdvancedInput = document.createElement('input')
    calAdvancedInput.setAttribute('type', 'checkbox')
    calAdvancedInput.classList.add('custom-control-input')
    calAdvancedInput.setAttribute('id', 'addImagesCalAdvancedCheckbox')
    calAdvancedInput.setAttribute('name', 'addImagesCalAdvancedCheckbox')
    calCheckDiv.appendChild(calAdvancedInput)

    var calAdvancedLabel = document.createElement('label')
    calAdvancedLabel.classList.add('custom-control-label')
    calAdvancedLabel.setAttribute('for', 'addImagesCalAdvancedCheckbox')
    calAdvancedLabel.innerHTML = 'Advanced'
    calCheckDiv.appendChild(calAdvancedLabel)

    var calBuilder = document.createElement('div')
    calBuilder.setAttribute('id', 'addImagesCalBuilder')
    calIdDiv.appendChild(calBuilder)

    var addFilesCalErrors = document.createElement('div')
    addFilesCalErrors.setAttribute('id', 'addFilesCalErrors')
    addFilesCalErrors.setAttribute('style', 'font-size: 80%; color: #DF691A')
    calOptionsSection.appendChild(addFilesCalErrors)

    addFilesDiv.appendChild(document.createElement('br'))

    $("#addImagesCalAdvancedCheckbox").change(function() {
        var calAdvancedCheckbox = document.getElementById('addImagesCalAdvancedCheckbox')
        var yesCal = document.getElementById('yesCalImagesES').checked

        if (calAdvancedCheckbox.checked && yesCal) {
            document.getElementById('addFilesCalErrors').innerHTML = ''
            buildCalBuilderRow()
        } else {
            document.getElementById('addFilesCalErrors').innerHTML = ''
            var addImagesCalBuilder = document.getElementById('addImagesCalBuilder')
            while (addImagesCalBuilder.firstChild) {
                addImagesCalBuilder.removeChild(addImagesCalBuilder.firstChild)
            }
            document.getElementById('addImagesCalCodeInput').value = ''
        }
    ...
    $("#addImagesCalCodeInput").change(function() {
        rescanSelectedFolder()
    })
...
document.getElementById('btnSaveSurvey').addEventListener('click', ()=>{
    ...
    */
    ...
    // Previous Yes/No + identifier validation (configurable regex):
    // if (emptySurvey || noCalImages) {
    //     newSurveyCalCode = 'None'
    // } else {
    //     newSurveyCalCode = document.getElementById('newSurveyCalCodeInput').value.trim()
    //     if (newSurveyCalCode === '') {
    //         legalCalCode = false
    ...
    if (emptySurvey) {
        ...
        if ((newSurveyStructureDiv!=null)&&(newSurveyStructureDiv.innerHTML == '')||(newSurveyStructureDiv.innerHTML == 'Malformed expression. Please try again.')||(newSurveyStructureDiv.innerHTML == 'Invalid structure. Please check your site and camera identifiers.')||(newSurveyStructureDiv.innerHTML.includes('identifier is invalid'))||(newSurveyStructureDiv.innerHTML == 'Invalid structure. Please check your site, camera and calibration identifiers.')||(newSurveyStructureDiv.innerHTML == 'Invalid structure. Please check your site, camera and calibration folders.')) {
            legalTGCode = false
            legalCamCode = false
...
function addFiles(){
    ...
    } else if (calCtx.calErrors) {
        calCtx.calErrors.innerHTML = ''
    }

    // Previous Yes/No + identifier validation (configurable regex):
    // if (calCtx.noCal.checked) {
    //     addImagesCalCode = 'None'
    // } else {
    //     addImagesCalCode = calCtx.calInput.value.trim()
    //     if (addImagesCalCode === '') {
    //         legalCalCode = false
    //         calCtx.calErrors.innerHTML = 'The calibration identifier field cannot be empty.'
    //     } else if (addImagesCalCode.endsWith('.*') || addImagesCalCode.endsWith('.+') ||
    //             addImagesCalCode.endsWith('.*[0-9]+') || addImagesCalCode.endsWith('.+[0-9]+')) {
    //         legalCalCode = false
    //         calCtx.calErrors.innerHTML = 'Your calibration identifier is invalid. Please try again or send an email for assistance.'
    //     } else {
    //         var calStruct = validateCalibrationStructure()
    //         if (!calStruct.valid) {
    //             legalCalCode = false
    ...
    if ((addImagesStructureDiv!=null)&&((addImagesStructureDiv.innerHTML == '')||(addImagesStructureDiv.innerHTML == 'Malformed expression. Please try again.')||(addImagesStructureDiv.innerHTML == 'Invalid structure. Please check your site and camera identifiers.')||(addImagesStructureDiv.innerHTML.includes('identifier is invalid'))||(addImagesStructureDiv.innerHTML == 'Invalid structure. Please check your site, camera and calibration identifiers.')||(addImagesStructureDiv.innerHTML == 'Invalid structure. Please check your site, camera and calibration folders.'))) {
        legalTGCode = false
        legalCamCode = false
        document.getElementById('addFilesErrors').innerHTML = 'Your specified site or camera identifiers are invalid. Please try again.'
    }

    if (legalTGCode&&legalInput&&structureCheckReady&&legalCamCode&&legalCalCode) {
        var formData = new FormData()
        formData.append("survey_id", selectedSurvey)
        ...
        // Always override survey.calibration_code to the fixed keyword on manage-files upload.
        formData.append("newSurveyCalCode", CALIBRATION_FOLDER_KEYWORD)
        formData.append("calCheckbox", 'true')
        // Previous optional submit:
        // if (!calCtx.noCal.checked) {
        //     formData.append("newSurveyCalCode", calCtx.calInput.value)
        //     formData.append("calCheckbox", calCtx.calAdvanced.checked.toString())
        // }
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                if (reply.status=='success') {
                    ...
                    if (document.getElementById('BrowserAdd').checked &&
                            typeof getSurveyCalibrationCode === 'function') {
                        pendingUploadCalibrationCode = getSurveyCalibrationCode()
                    }
...
function buildCalibrationImages() {
    ...
    document.getElementById('editCalImages').addEventListener('click', () => {
        document.getElementById('calModeDescription').innerHTML = '<i>View and edit bounding boxes and distances, or stage images for deletion.</i>'
    ...
    // Default to Edit Calibration Images
    document.getElementById('editCalImages').checked = true
    document.getElementById('editCalImages').click()
}
...
function filterCalUploadAgainstExisting(files, existingDistances, otherStagedSets, cameragroupId) {
    /** Keep files whose parsed filename distance is not already in DB (or staged) for this camera. */
    var taken = {}
    var i, j, d
...
function matchCameragroupForCalRelativePath(cameraRelativePath, cameragroups) {
    /** Match local path above a calibration folder to an existing cameragroup relative_path. */
    if (!cameraRelativePath || !cameragroups) return null
    var parts = cameraRelativePath.split('/').filter(function (p) { return p })
    var candidates = []
    for (var i = 0; i < parts.length; i++) {
        candidates.push(parts.slice(i).join('/'))
    }
    for (var c = 0; c < candidates.length; c++) {
        for (var g = 0; g < cameragroups.length; g++) {
            if (cameragroups[g].relative_path && cameragroups[g].relative_path === candidates[c]) {
                return cameragroups[g]
            }
        }
    }
    // Fallback: last folder name equals cameragroup name
    if (parts.length > 0) {
        var last = parts[parts.length - 1]
        for (var n = 0; n < cameragroups.length; n++) {
            if (cameragroups[n].name === last) return cameragroups[n]
        }
    }
    return null
}
...
function scanBulkCalibrationFolderFiles(fileList) {
    ...
    for (var i = 0; i < fileCount; i++) {
        ...
        if (byCamera[cameraRelativePath].seen[distance]) continue
    ...
    var groups = []
    var emptyPaths = []
    Object.keys(byCamera).forEach(function (cameraRelativePath) {
        var g = byCamera[cameraRelativePath]
        if (g.files.length === 0) {
        ...
        groups.push({
            ...
            files: pairs.map(function (x) { return x.file }),
            distances: pairs.map(function (x) { return x.distance })
        })
    })
...
function uploadOneCalFileToStaging(file, cameragroupId) {
    ...
    return fetch('/getCalibrationUploadUrl', {
        ...
        if (!response.ok) throw new Error('Could not get upload URL for ' + file.name)
        return response.json()
    }).then(function (data) {
        if (!data || data.status !== 'success' || !data.url) {
            throw new Error((data && data.message) || ('Upload URL failed for ' + file.name))
        }
        return fetch(data.url, {
            method: 'PUT',
            headers: { 'Content-Type': data.contentType || file.type || 'image/jpeg' },
            body: file,
        }).then(function (putResp) {
            if (!putResp.ok) throw new Error('Failed to upload ' + file.name + ' to storage')
...
function buildCalUploadMode() {
    ...
    singleRadioWrap.setAttribute('class', 'custom-control custom-radio custom-control-inline')
    modeCol.appendChild(singleRadioWrap)
    var singleInput = document.createElement('input')
    singleInput.type = 'radio'
    singleInput.classList.add('custom-control-input')
    singleInput.id = 'calUploadModeSingle'
    singleInput.name = 'calUploadMode'
    singleInput.checked = true
    singleRadioWrap.appendChild(singleInput)
    var singleLabel = document.createElement('label')
    singleLabel.classList.add('custom-control-label')
    singleLabel.setAttribute('for', 'calUploadModeSingle')
    singleLabel.innerHTML = 'Single camera'
    singleRadioWrap.appendChild(singleLabel)

    var bulkRadioWrap = document.createElement('div')
    bulkRadioWrap.setAttribute('class', 'custom-control custom-radio custom-control-inline')
    modeCol.appendChild(bulkRadioWrap)
    var bulkInput = document.createElement('input')
    bulkInput.type = 'radio'
    bulkInput.classList.add('custom-control-input')
    bulkInput.id = 'calUploadModeBulk'
    bulkInput.name = 'calUploadMode'
    bulkRadioWrap.appendChild(bulkInput)
    var bulkLabel = document.createElement('label')
    bulkLabel.classList.add('custom-control-label')
    bulkLabel.setAttribute('for', 'calUploadModeBulk')
    bulkLabel.innerHTML = 'Bulk from folder structure'
    bulkRadioWrap.appendChild(bulkLabel)
    ...
    div.appendChild(singlePanel)

    var bulkPanel = document.createElement('div')
    bulkPanel.id = 'calUploadBulkPanel'
    bulkPanel.hidden = true
    div.appendChild(bulkPanel)

    // ── Single-camera panel (existing flow) ─────────────────────────
    var row1 = document.createElement('div')
    row1.classList.add('row')
    singlePanel.appendChild(row1)
    var col1 = document.createElement('div')
    col1.classList.add('col-lg-6')
    ...
    bulkPanel.appendChild(brow)
    var bcol = document.createElement('div')
    bcol.classList.add('col-lg-10')
    ...
    var bulkFolderInput = document.createElement('input')
    bulkFolderInput.type = 'file'
    bulkFolderInput.id = 'calUploadBulkFolderInput'
    bulkFolderInput.setAttribute('webkitdirectory', '')
    bulkFolderInput.setAttribute('multiple', '')
    bulkFolderInput.style.display = 'none'
    bcol.appendChild(bulkFolderInput)

    var bulkScanBtn = document.createElement('button')
    ...
    var bulkStatus = document.createElement('div')
    bulkStatus.id = 'calUploadBulkStatusMsg'
    bulkStatus.setAttribute('style', 'font-size: 80%; margin-top: 8px')
    bcol.appendChild(bulkStatus)

    var bulkMatchDiv = document.createElement('div')
    bulkMatchDiv.id = 'calUploadBulkMatchDiv'
    bulkMatchDiv.style.marginTop = '10px'
    bcol.appendChild(bulkMatchDiv)

    var bulkStageBtn = document.createElement('button')
    bulkStageBtn.id = 'calUploadBulkStageBtn'
    bulkStageBtn.classList.add('btn', 'btn-primary', 'btn-block')
    bulkStageBtn.style.maxWidth = '360px'
    bulkStageBtn.style.marginTop = '8px'
    bulkStageBtn.innerHTML = 'Stage matched sets'
    bulkStageBtn.disabled = true
    bulkStageBtn.hidden = true
    bcol.appendChild(bulkStageBtn)

    var pendingBulkMatches = []
    ...
    bulkInput.addEventListener('change', function () { if (bulkInput.checked) setUploadMode('bulk') })
    ...
    folderInput.onchange = function () {
        ...
        fetchExistingCalDistances(cgId, function (existingDistances) {
            ...
            addStagedCalUploadSet(cgId, cgName, filtered.files, filtered.distances, false)
    ...
    bulkScanBtn.onclick = function () {
        bulkStatus.innerHTML = ''
        bulkMatchDiv.innerHTML = ''
        bulkStageBtn.hidden = true
        bulkStageBtn.disabled = true
        pendingBulkMatches = []
    ...
    bulkFolderInput.onchange = function () {
        ...
        for (var i = 0; i < scanned.groups.length; i++) {
            var g = scanned.groups[i]
            var cg = matchCameragroupForCalRelativePath(g.cameraRelativePath, surveyCameragroups)
            if (cg) {
                pendingBulkMatches.push({
                    cameragroupId: cg.id,
                    cameragroupName: cg.name,
                    cameraRelativePath: g.cameraRelativePath,
                    matchedPath: cg.relative_path,
                    files: g.files,
                    distances: g.distances
                })
            } else {
                unmatched.push(g.cameraRelativePath)
            }
        }
        ...
        if (scanned.emptyPaths.length) summary += ', ' + scanned.emptyPaths.length + ' empty/invalid'
        bulkStatus.innerHTML = '<i>' + summary + '.</i>'

        bulkStageBtn.hidden = false
        bulkStageBtn.disabled = pendingBulkMatches.length === 0
    }

    bulkStageBtn.onclick = function () {
        if (pendingBulkMatches.length === 0) return
        bulkStageBtn.disabled = true
        var remaining = pendingBulkMatches.length
        var stagedCount = 0
        var skippedAllExist = 0

        function finishOne() {
            remaining -= 1
            if (remaining > 0) return
            renderStagedCalUploadSets(stagedDiv)
            bulkStatus.innerHTML = '<i>Staged ' + stagedCount + ' camera set(s)' +
                (skippedAllExist ? '; skipped ' + skippedAllExist + ' with no new distances' : '') +
                '. Click Save Changes to upload and apply.</i>'
            bulkStageBtn.disabled = pendingBulkMatches.length === 0
            pendingBulkMatches = []
            bulkStageBtn.hidden = true
            bulkMatchDiv.innerHTML = ''
        ...
        pendingBulkMatches.forEach(function (row) {
            fetchExistingCalDistances(row.cameragroupId, function (existingDistances) {
                var filtered = filterCalUploadAgainstExisting(
                    row.files, existingDistances, stagedCalUploadSets, row.cameragroupId
                )
                if (filtered.files.length === 0) {
                    skippedAllExist += 1
                    finishOne()
...
function areAllCurrentCalImagesStagedForDeletion() {
    if (!calImages || calImages.length === 0) return false
    for (var i = 0; i < calImages.length; i++) {
        if (deletedCalImages.indexOf(calImages[i].id) === -1) return false
    }
    return true
}
...
function updateCalDeleteButtonStates() {
    ...
    if (stageBtn && calImages.length > 0) {
        ...
        if (isStaged) {
            stageBtn.innerHTML = 'Unstage Deletion'
    ...
    if (camBtn) {
        ...
        if (areAllCurrentCalImagesStagedForDeletion()) {
            ...
            camBtn.classList.add('btn-warning')
        } else {
...
function stageCalibrationDeletion() {
    ...
    if (!calImages || calImages.length === 0) return
...
function stageCalibrationCameragroupDeletion() {
    /** Stages or un-stages all calibration images for the currently selected cameragroup. */
    if (!calImages || calImages.length === 0) return

    if (areAllCurrentCalImagesStagedForDeletion()) {
        for (var i = 0; i < calImages.length; i++) {
            var idx = deletedCalImages.indexOf(calImages[i].id)
            if (idx !== -1) deletedCalImages.splice(idx, 1)
        }
        document.getElementById('calStatusMsg').innerHTML =
            '<i>Unstaged deletion for all ' + calImages.length + ' image(s) on this camera.</i>'
    } else {
        for (var j = 0; j < calImages.length; j++) {
            var cal_id = calImages[j].id
            if (deletedCalImages.indexOf(cal_id) === -1) {
                deletedCalImages.push(cal_id)
            }
            recordCalImageCameragroup(cal_id)
        }
...
function buildCalEditMode() {
    ...
    var stageCamBtn = document.createElement('button')
    stageCamBtn.id = 'calStageDeleteCameraBtn'
...
function updateCalMap() {
    ...
    document.getElementById('mapTitle_cal').innerHTML = img.url.split('/').slice(2).filter(function (p) {
        return p !== '_calibration_'
    }).join('/')
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (calCameras.length === 0 && selectedCalMode === 'edit') {
                ...
                var col = document.createElement('div')
                col.classList.add('col-lg-12', 'd-flex', 'align-items-center', 'justify-content-center')
                row.appendChild(col)
                var h6 = document.createElement('h6')
                h6.innerHTML = 'No calibration images available to edit.'
                col.appendChild(h6)
                return
            }
...
function isAddFilesCalibrationContext() {
    return document.getElementById('addImagesTGCode') != null
}

function getCalibrationUiContext() {
    if (isAddFilesCalibrationContext()) {
        return {
            noCal: document.getElementById('noCalImagesES'),
            yesCal: document.getElementById('yesCalImagesES'),
            calInput: document.getElementById('addImagesCalCodeInput'),
            calAdvanced: document.getElementById('addImagesCalAdvancedCheckbox'),
            calBuilder: document.getElementById('addImagesCalBuilder'),
            calErrors: document.getElementById('addFilesCalErrors'),
            calDetailsDiv: document.getElementById('calDetailsDivES'),
            calIdDiv: document.getElementById('calIdDivES'),
        }
    }
    return {
        noCal: document.getElementById('noCalImages'),
        yesCal: document.getElementById('yesCalImages'),
        calInput: document.getElementById('newSurveyCalCodeInput'),
        calAdvanced: document.getElementById('calAdvancedCheckbox'),
        calBuilder: document.getElementById('newSurveyCalBuilder'),
        calErrors: document.getElementById('newSurveyCalErrors'),
        calDetailsDiv: document.getElementById('calDetailsDiv'),
        calIdDiv: document.getElementById('calIdDiv'),
    }
}

function isCalibrationYesSelected() {
    // Treat as "yes" whenever calibration folders were found.
    // Legacy Yes/No UI is hidden; configurable identifiers can be restored later.
    return globalCalibrationFolderCount > 0
}

function isBrowserFolderUploadContext() {
    return (document.getElementById('BrowserUpload') && document.getElementById('BrowserUpload').checked) ||
...
function updateCalDiv() {
    // Legacy Yes/No UI handler. Section is hidden; auto-detect scans on folder select instead.
    // Kept so restoring configurable identifiers later is straightforward.
    var ctx = getCalibrationUiContext()
    if (!ctx.noCal) return
    var noCal = ctx.noCal.checked

    if (noCal) {
        if (ctx.calDetailsDiv) ctx.calDetailsDiv.hidden = true
        if (ctx.calIdDiv) ctx.calIdDiv.hidden = true
        ...
        if (ctx.calDetailsDiv) ctx.calDetailsDiv.hidden = false
        if (ctx.calIdDiv) ctx.calIdDiv.hidden = false
        if (ctx.calAdvanced && ctx.calAdvanced.checked) {
            buildCalBuilderRow()
        }
        rescanSelectedFolder()
        if (!uploadDirHandle) {
...
function getCameraLevelIndexForPath(parts) {
    ...
    if (camLvlFolder) {
        ...
        return camFolder ? parseInt(camFolder.dataset.index, 10) : -1
    }
...
function validateCalibrationStructure() {
    /** Validates auto-detected calibration folders when any are present. */
    // No calibration folders → operate as before (no calib mention / no errors).
    if (!globalCalibrationFolderPaths || globalCalibrationFolderPaths.length === 0) {
    ...
    if (isBrowserFolderUploadContext() || isBucketFolderUploadContext()) {
        for (var i = 0; i < globalCalibrationFolderPaths.length; i++) {
            ...
            var calIndex = parts.length - 1
            var cameraLevel = getCameraLevelIndexForPath(parts)

            if (cameraLevel < 0) {
                return {
                    valid: false,
                    message: 'Select site and camera folder levels before using calibration.'
                }
            }
            if (calIndex <= cameraLevel) {
                return {
                    valid: false,
...
function getSurveyCalibrationCode() {
     ...
     *   var ctx = getCalibrationUiContext()
     *   if (!ctx.noCal || ctx.noCal.checked) return null
     *   var code = ctx.calInput.value.trim()
     *   if (code === '') return null
     *   if (!ctx.calAdvanced.checked) code = code + '[0-9]*'
     *   return code
     */
    return CALIBRATION_FOLDER_KEYWORD
}

function siteCameraHasCalibration(siteName, camName) {
    /** True if a calibration folder sits under this site/camera in the selected tree. */
    ...
    for (var i = 0; i < globalCalibrationFolderPaths.length; i++) {
        ...
        if (camName == null || camName === siteName) {
            // camSameAsSite: cal folder under the site is enough
            if (siteIdx < parts.length - 1) return true
        } else {
            var camIdx = parts.indexOf(camName)
            if (camIdx > siteIdx && camIdx < parts.length - 1) return true
        }
    }
    return false
}
...
$("#calAdvancedCheckbox").change(function() {
    var calAdvancedCheckbox = document.getElementById('calAdvancedCheckbox');
    var yesCal = document.getElementById('yesCalImages').checked;
    ...
    if (calAdvancedCheckbox.checked && yesCal) {
        ...
        // Optional: clear input when leaving advanced mode
        document.getElementById('newSurveyCalCodeInput').value = '';
    }
...
function updateCalCode() {
    /** Builds calibration regex from builder rows into newSurveyCalCodeInput. */
    var calCode = '';
    var charSelectCals = document.querySelectorAll('[id^=charSelectCal-]');
    for (let i = 0; i < charSelectCals.length; i++) {
        var IDNum = charSelectCals[i].id.split('-')[charSelectCals[i].id.split('-').length - 1];
        var selection = charSelectCals[i].options[charSelectCals[i].selectedIndex].text;
        ...
        if (selection == 'Any Digit') {
            ...
            var customCharactersCal = document.getElementById('customCharactersCal-' + IDNum).value;
            var CustomCharSelectCal = document.getElementById('CustomCharSelectCal-' + IDNum);
            selection = CustomCharSelectCal.options[CustomCharSelectCal.selectedIndex].text;
            if (selection == 'Exactly') {
                calCode += customCharactersCal;
            } else if (selection == 'Or') {
                calCode += '[' + customCharactersCal + ']';
            }
        }

        var occurrenceSelectCal = document.getElementById('occurrenceSelectCal-' + IDNum);
        selection = occurrenceSelectCal.options[occurrenceSelectCal.selectedIndex].text;
        if (selection == 'Zero or More') {
            calCode += '*';
        } else if (selection == 'Zero or One') {
            calCode += '?';
        } else if (selection == 'One or More') {
            ...
            var customOccurrenceCal = document.getElementById('customOccurrenceCal-' + IDNum).value;
            calCode += '{' + customOccurrenceCal.toString() + '}';
        }
    }

    var ctx = getCalibrationUiContext()
    ctx.calInput.value = calCode
    if (calCode.endsWith('.*') || calCode.endsWith('.+') ||
            calCode.endsWith('.*[0-9]+') || calCode.endsWith('.+[0-9]+')) {
        ctx.calErrors.innerHTML =
            'Your calibration identifier is invalid. Please try again or contact us for assistance.'
    } else {
        ctx.calErrors.innerHTML = ''
    }
    rescanSelectedFolder()
...
function buildCalBuilderRow() {
    /** Builds a row for the calibration code builder. */

    IDNum = getIdNumforNext('charSelectCal')

    if (IDNum==0) {
        // Initialising - build headings
        calBuilder = document.getElementById('addImagesCalBuilder') || document.getElementById('newSurveyCalBuilder')

        while(calBuilder.firstChild){
            calBuilder.removeChild(calBuilder.firstChild);
        }

        calBuilder.append(document.createElement('br'))

        calBuilderRows = document.createElement('div')
        calBuilderRows.setAttribute('id','calBuilderRows')
        calBuilder.append(calBuilderRows)

        // Add add-row button
        calBuilderBtnRow = document.createElement('div')
        calBuilderBtnRow.classList.add('row')
        calBuilder.append(calBuilderBtnRow)

        col1 = document.createElement('div')
        col1.classList.add('col-lg-11')
        calBuilderBtnRow.appendChild(col1)

        col2 = document.createElement('div')
        col2.classList.add('col-lg-1')
        calBuilderBtnRow.appendChild(col2)  
        ...
        calBuilderRows = document.getElementById('calBuilderRows')
    }
    ...
    $("#"+selectID).change( function(wrapIDNum) {
        return function() {
            select = document.getElementById('charSelectCal-'+wrapIDNum)
            selection = select.options[select.selectedIndex].text
            if (selection == 'Custom Set') {
                //Build custom row
                col2 = document.getElementById('calBuilderCol2-'+wrapIDNum)

                input = document.createElement('input')
                input.setAttribute('type','text')
                input.classList.add('form-control')
                input.required = true
                input.setAttribute('id','customCharactersCal-'+wrapIDNum)
                col2.appendChild(input)

                $("#customCharactersCal-"+wrapIDNum).change( function() {
                    updateCalCode()
                })

                col3 = document.getElementById('calBuilderCol3-'+wrapIDNum)
                
                select = document.createElement('select')
                select.classList.add('form-control')
                select.setAttribute('id','CustomCharSelectCal-'+wrapIDNum)
                col3.appendChild(select)

                $("#CustomCharSelectCal-"+wrapIDNum).change( function() {
                    updateCalCode()
                })
                ...
                // Remove any custom row
                div = document.getElementById('calBuilderCol2-'+wrapIDNum)
                while(div.firstChild){
                    div.removeChild(div.firstChild);
                }

                div = document.getElementById('calBuilderCol3-'+wrapIDNum)
                while(div.firstChild){
                    div.removeChild(div.firstChild);
                }
            }
    ...
    col2 = document.createElement('div')
    col2.classList.add('col-lg-3')
    col2.setAttribute('id','calBuilderCol2-'+IDNum)
    row.appendChild(col2)

    // Custom Character Operation Column
    col3 = document.createElement('div')
    col3.classList.add('col-lg-2')
    col3.setAttribute('id','calBuilderCol3-'+IDNum)
    row.appendChild(col3)
    ...
    selectID = 'occurrenceSelectCal-'+IDNum
    select = document.createElement('select')
    select.classList.add('form-control')
    select.setAttribute('id',selectID)
    col4.appendChild(select)
    ...
    $("#"+selectID).change( function(wrapIDNum) {
        return function() {
            select = document.getElementById('occurrenceSelectCal-'+wrapIDNum)
            selection = select.options[select.selectedIndex].text
            if (selection == 'Custom Count') {
                //Build custom row
                col5 = document.getElementById('calBuilderCol5-'+wrapIDNum)

                input = document.createElement('input')
                input.setAttribute('type','text')
                input.classList.add('form-control')
                input.required = true
                input.setAttribute('id','customOccurrenceCal-'+wrapIDNum)
                col5.appendChild(input)

                $("#customOccurrenceCal-"+wrapIDNum).change( function() {
                    updateCalCode()
                })
            } else {
                // Remove any custom row
                div = document.getElementById('calBuilderCol5-'+wrapIDNum)
                while(div.firstChild){
                    div.removeChild(div.firstChild);
                }
            }
    ...
    col5 = document.createElement('div')
    col5.classList.add('col-lg-2')
    col5.setAttribute('id','calBuilderCol5-'+IDNum)
    row.appendChild(col5)
...
function updateSurveyStructure(){
    ...
    var calOk = validateCalibrationStructure().valid
    var showCalCoverage = globalCalibrationFolderCount > 0

    if (structurePages.length > 0 && calOk) {
        infoDiv.innerHTML = ''
        ...
        if (camSameAsSite) {
            ...
            for (let i=0;i<tags.length;i++) {
                var cams = globalSurveyStructure[structure_page][tags[i]]
                var siteHasMissingCam = false
                if (showCalCoverage) {
                    for (let n=0;n<cams.length;n++) {
                        if (!siteCameraHasCalibration(tags[i], cams[n])) {
                            siteHasMissingCam = true
                            break
                        }
                    }
                }
