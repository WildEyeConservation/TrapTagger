...
@celery.task(bind=True,max_retries=5,ignore_result=True)
def finish_knockdown(self,rootImageID, task, current_user_id, reclusteringTimestamp=None, lastImageID=None):
    '''
    Celery task for marking a camera as knocked down. Combines all images into a new cluster, and reclusters the images from the other cameras.
        ...
        Parameters:
            ...
            reclusteringTimestamp (datetime): The starting timestamp for the reclustering process
...
def unknock_cluster(self,image_id, label_id, user_id, task_id):
    ...
    try:
        ...
        if trapgroup.queueing:
            trapgroup.queueing = False
            finish_knockdown.apply_async(kwargs={'rootImageID':image_id, 'task':task_id, 'current_user_id':user_id})
            ...
            if task.status=='PROGRESS':
                ...
                # task is in wrap up after knockdown analysis. Check for work and launch if needed else free task
                queueing = db.session.query(Trapgroup).filter(Trapgroup.survey_id==task.survey_id).filter(Trapgroup.queueing==True).count()
                processing = db.session.query(Trapgroup).filter(Trapgroup.survey_id==task.survey_id).filter(Trapgroup.processing==True).count()

                if not queueing and not processing:
...
def updateLabelCompletionStatus(task_id):
    ...
    count = db.session.query(Cluster.id).filter(Cluster.task_id==task_id)
...
def taggingLevelSQ(sq,taggingLevel,isBounding,task_id,trapgroup_id=None):
    ...
    if (taggingLevel == '-1') or (taggingLevel == '0'):
        ...
        classifier_id, dataType = db.session.query(Classifier.id, Survey.type).join(Survey).join(Task).filter(Task.id==task_id).first()
        validClassifications = db.session.query(Translation.classification)\
                                .filter(Translation.task_id==task_id)\
                                .filter(Translation.label_id!=GLOBALS.nothing_id)\
                                .distinct()
        class_ok = and_(
            ClassificationLabel.id!=None,
            Detection.class_score>ClassificationLabel.threshold,
            Detection.classification.in_(validClassifications)
        )
        detSQ = rDets(db.session.query(
                                Cluster.id.label('cluster_id'),
                                Detection.classification.label('classification'),
                                func.count(case((class_ok, Detection.id))).label('class_count'),
                                func.count(Detection.id).label('det_count')
                            )\
                            .join(Image,Cluster.images)\
                            .join(Detection)\
                            .outerjoin(ClassificationLabel,and_(ClassificationLabel.classification==Detection.classification,ClassificationLabel.classifier_id==classifier_id))\
                            .filter(Cluster.task_id==task_id)\
                            .filter(((Detection.right-Detection.left)*(Detection.bottom-Detection.top)) > Config.CLASSIFICATION_DET_AREA[dataType]))
        if trapgroup_id: detSQ = detSQ.join(Camera).filter(Camera.trapgroup_id==trapgroup_id)
        detSQ = detSQ.group_by(Cluster.id,Detection.classification).subquery()
        # Window must run over all classification groups so the denominator stays correct
        totalsSQ = db.session.query(
                                detSQ.c.cluster_id.label('cluster_id'),
                                detSQ.c.classification.label('classification'),
                                detSQ.c.class_count.label('count'),
                                func.sum(detSQ.c.det_count).over(partition_by=detSQ.c.cluster_id).label('total_count')
                            ).subquery()
        classificationSQ = db.session.query(
                                totalsSQ.c.cluster_id.label('cluster_id'),
                                totalsSQ.c.classification.label('classification')
                            )\
                            .filter(totalsSQ.c.count>1)\
                            .filter((totalsSQ.c.count/totalsSQ.c.total_count)>Config.MIN_CLASSIFICATION_RATIO[dataType])\
                            .subquery()
        labelstableSQ = db.session.query(labelstable.c.cluster_id.label('cluster_id'),Translation.classification.label('classification'))\
                                .join(Translation,Translation.label_id==labelstable.c.label_id)\
                                .join(Cluster,Cluster.id==labelstable.c.cluster_id)\
                                ...
                                .filter(Translation.task_id==task_id)\
                                .subquery()
        sq = sq.join(classificationSQ,classificationSQ.c.cluster_id==Cluster.id)\
                                .outerjoin(labelstableSQ,and_(labelstableSQ.c.cluster_id==Cluster.id,labelstableSQ.c.classification==classificationSQ.c.classification))\
                                .filter(labelstableSQ.c.classification==None)\
                                .filter(~Cluster.labels.any(Label.id==GLOBALS.knocked_id))
...
def inspect_celery(include_spam=False,include_reserved=False,include_scheduled=False):
    ...
    spam = ['importImages','.detection','.classify','runClassifier','processCameraStaticDetections', 'process_video_batch','processStaticWindow', 
            ...
            'updateTrapgroupDetectionRatings', 'extract_zip', 'prep_required_images']
...
def cluster_timestampless(task_id,trapgroup_id,starting_last_cluster_id,query_limit):
    ...
    while current_row!=final_row:
        ...
        for image, video_id, notes, user_id, timestamp, checked, er_ids, row_number in imageData:
            ...
            if (cluster_user_id is None) and (user_id!=admin.id): cluster_user_id = user_id
            if (cluster_timestamp is None) and (user_id!=admin.id): cluster_timestamp = timestamp
...
def time_based_clustering(task_id,trapgroup_id,query_limit,timestamp=None):
    '''
...
def det_presence_clustering(task_id,trapgroup_id,starting_last_cluster_id,query_limit,timestamp=None):
    '''
    ...
    for cameragroup_id, det_presence, start, end, image_ids, row_number in cameragroupClusters:
        ...
        for cg_id in cameragroup_ids:
            if (start-last_sighting_time[cg_id]) > timedelta(minutes=Config.MAX_CLUSTER_MINUTES)/2:
                last_sighting_index[cg_id] = False
        ...
        if (cluster_image_ids and (was_empty != is_empty)) or (row_number==last_row):
            ...
            for image in cluster_images:
                ...
                if (cluster_user_id is None) and (image_user_id!=admin.id): cluster_user_id = image_user_id
                if (cluster_timestamp is None) and (image_user_id!=admin.id): cluster_timestamp = image_timestamp
...
def delete_old_clusters(task_id,trapgroup_id,query_limit):
    ''' Leaves only the latest cluster for a given task and trapgroup. '''
    ...
    try:
        ...
        if '-5' in taggingLevel:
            ...
            if '-3' in taggingLevel:
                ...
                #         )
                sq = db.session.query(Cluster).filter(Cluster.task_id==task_id)
                sq = taggingLevelSQ(sq,'-3',False,task_id)
                clusters = sq.distinct().all()
...
def calculate_individual_best_detection(individual_ids):
    ...
    .join(Camera, Image.camera)\
    .filter(Individual.id.in_(individual_ids)))\
    .distinct().all()

    for d in detections:
        individual_id, det_id, raw_score, top, bottom, left, right, flank, timestamp, filename, path = d
        # Calculate scores
        if individual_id not in scores:
            scores[individual_id] = {}
            det_data[individual_id] = {}
        scores[individual_id][det_id] = calculate_detection_score(raw_score,top,bottom,left,right,flank,timestamp)
        det_data[individual_id][det_id] = {
            'det_id': det_id,
            'filename': filename,
            'path': path,
            'top': top,
    ...
    # Find the best detection for each individual
    for individual_id in individual_ids:
        score = scores.get(individual_id, None)
        if score:
            best_detection = max(score, key=score.get)
            best_detections[individual_id] = det_data[individual_id][best_detection]
        else:
            best_detections[individual_id] = None

    return best_detections

def calculate_detection_score(raw_score,top,bottom,left,right,flank,timestamp):
    '''Calculates the detection score based on the given parameters.'''
