...
# #TODO: STILL UPDATE/CHECK (Copy Survey)
# @celery.task(bind=True,max_retries=0,ignore_result=True)
# def copy_survey(self,survey_id, new_org_id, tasks=False, queue='parallel'):
#     '''Copies a survey and all of its associated data.'''
#     try:
#         survey = db.session.query(Survey).get(survey_id)
#         if survey.organisation.id==new_org_id:
#             new_survey = Survey(name=survey.name+'_copy', organisation_id=new_org_id, status='Processing')
#             db.session.add(new_survey)
#         else:
#             new_survey = Survey(name=survey.name, organisation_id=new_org_id, status='Processing')
#             db.session.add(new_survey)
#         db.session.commit()
...
#                 new_task = Task(name=task.name, survey_id=new_survey.id, status='Ready')
#                 db.session.add(new_task)
#                 db.session.commit()
#                 task_ids.append(task.id)
#                 new_task_ids.append(new_task.id)
#         else:
#             new_task = Task(name='default', survey_id=new_survey.id, status='Ready')
#             db.session.add(new_task)
#             db.session.commit()
#             task_ids = [task.id for task in survey.tasks if task.name=='default']
#             new_task_ids = [new_task.id]

#         # Copy S3 
#         new_folder = new_survey.organisation.folder
#         if new_folder and new_folder.lower() != survey.organisation.folder.lower():
#             folder = new_folder
#             # Copy s3 folder
#             s3Folder = survey.organisation.folder + '/' + survey.name
#             new_S3Folder = new_folder + '/' + new_survey.name
#             for dirpath, folders, filenames in s3traverse(Config.BUCKET, s3Folder):
#                 for filename in filenames:
#                     new_key = dirpath.replace(s3Folder,new_S3Folder) + '/' + filename
#                     old_key = dirpath + '/' + filename
#                     GLOBALS.s3client.copy_object(Bucket=Config.BUCKET, CopySource={'Bucket': Config.BUCKET, 'Key': old_key}, Key=new_key)

#             # Copy S3 comp folder
#             s3CompFolder = survey.organisation.folder + '-comp/' + survey.name 
#             new_S3CompFolder = new_folder + '-comp/' + new_survey.name
#             for dirpath, folders, filenames in s3traverse(Config.BUCKET, s3CompFolder):
#                 for filename in filenames:
#                     new_key = dirpath.replace(s3CompFolder,new_S3CompFolder) + '/' + filename
#                     old_key = dirpath + '/' + filename
#                     GLOBALS.s3client.copy_object(Bucket=Config.BUCKET, CopySource={'Bucket': Config.BUCKET, 'Key': old_key}, Key=new_key)

#         else:
#             folder = survey.organisation.folder
...
#             copy_trapgroups.apply_async(kwargs={'trapgroup_id':trapgroup_id, 'new_survey_id': new_survey.id, 'folder': survey.organisation.folder, 'new_folder': folder, 'task_ids': task_ids, 'new_task_ids': new_task_ids}, queue=queue)
...
#         # Copy DetSimilarities
#         # det_similarities = db.session.query(DetSimilarity).join(Detection).join(Image).join(Camera).join(Trapgroup).filter(Trapgroup.survey_id==survey_id).all()
#         # for det_similarity in det_similarities:
#         #     new_detection_1 = detection_map[det_similarity.detection_1]
#         #     new_detection_2 = detection_map[det_similarity.detection_2]
#         #     new_det_similarity = DetSimilarity(detection_1=new_detection_1, detection_2=new_detection_2, score=det_similarity.score)
#         #     db.session.add(new_det_similarity)
#         # db.session.commit()

#         # # Copy Individuals 
#         # individual_map = {}
#         # for task_id in task_ids:
#         #     new_task_id = new_task_ids[task_ids.index(task_id)]
#         #     individuals = db.session.query(Individual).filter(Individual.tasks.any(Task.id==task_id)).all()
#         #     for individual in individuals:
#         #         new_individual = db.session.query(Individual).filter(Individual.tasks.any(Task.id==new_task_id)).filter(Individual.species==individual.species).filter(Individual.name==individual.name).first()
#         #         if not new_individual:
#         #             new_individual = Individual(task_id=new_task_id, status=individual.status, notes=individual.notes, species=individual.species, name=individual.name)
#         #             db.session.add(new_individual)

#         #             task = db.session.query(Task).get(new_task_id)
#         #             new_individual.tasks.append(task)

#         #         individual_detection_ids = [detection_map[detection.id] for detection in individual.detections]
#         #         individual_detections = db.session.query(Detection).filter(Detection.id.in_(individual_detection_ids)).all()
#         #         new_individual.detections.extend(individual_detections)

#         #         for tag in individual.tags:
#         #             new_tag = db.session.query(Tag).filter(Tag.description==tag.description).filter(Tag.task_id==new_task_id).first()
#         #             if not new_tag:
#         #                 new_tag = Tag(description=tag.description, task_id=new_task_id, hotkey=tag.hotkey)
#         #                 db.session.add(new_tag)
#         #             new_individual.tags.append(new_tag)

#         #         db.session.commit()
#         #         individual_map[individual.id] = new_individual.id

#         # # Copy IndSimilarities
#         # individual_similarities = db.session.query(IndSimilarity).join(Individual, or_(IndSimilarity.individual_1_id==Individual.id, IndSimilarity.individual_2_id==Individual.id)).filter(Individual.tasks.any(Task.id.in_(task_ids))).all()
#         # for individual_similarity in individual_similarities:
#         #     new_individual_1 = individual_map[individual_similarity.individual_1]
#         #     new_individual_2 = individual_map[individual_similarity.individual_2]
#         #     detection_1 = detection_map[individual_similarity.detection_1]
#         #     detection_2 = detection_map[individual_similarity.detection_2]
#         #     new_individual_similarity = IndSimilarity(individual_1=new_individual_1, individual_2=new_individual_2, detection_1=detection_1, detection_2=detection_2, score=individual_similarity.score, old_score=individual_similarity.old_score, skipped=individual_similarity.skipped)
#         #     db.session.add(new_individual_similarity)
...
# #TODO: STILL UPDATE/CHECK (Copy Survey)
# @celery.task(bind=True,max_retries=0,ignore_result=True)
# def copy_trapgroups(self, trapgroup_id,new_survey_id,task_ids,new_task_ids,folder,new_folder):
#     '''Copies a trapgroup and all of its associated data.'''
#     try:
#         trapgroup = db.session.query(Trapgroup).get(trapgroup_id)
#         new_trapgroup = db.session.query(Trapgroup).filter(Trapgroup.survey_id==new_survey_id).filter(Trapgroup.tag==trapgroup.tag).first()
#         if not new_trapgroup:
#             new_trapgroup = Trapgroup(tag=trapgroup.tag, survey_id=new_survey_id, latitude=trapgroup.latitude, longitude=trapgroup.longitude, altitude=trapgroup.altitude)
#             db.session.add(new_trapgroup)
#             db.session.commit()
#         new_trapgroup_id = new_trapgroup.id

#         print('Copying trapgroup: {}'.format(trapgroup.tag))
...
#         print('Copying cameras: {}'.format(len(cameras)))
#         for camera in cameras:
#             camera_path = camera.path.replace(folder, new_folder)
#             new_camera = db.session.query(Camera).filter(Camera.path==camera_path).filter(Camera.trapgroup_id==new_trapgroup_id).first()
#             if not new_camera:
#                 new_camera = Camera(path=camera_path, trapgroup_id=new_trapgroup_id)
#                 db.session.add(new_camera)

#             # Copy cameragroup 
#             new_cameragroup = db.session.query(Cameragroup).join(Camera).join(Trapgroup).filter(Trapgroup.id==new_trapgroup_id).filter(Cameragroup.name==camera.cameragroup.name).first()
#             if not new_cameragroup:
#                 new_cameragroup = Cameragroup(name=camera.cameragroup.name)
#                 db.session.add(new_cameragroup)
#             new_cameragroup.cameras.append(new_camera)
...
#             print('Copying images: {}'.format(len(images)))
#             image_count = 0
#             for image in images:
#                 image_count += 1
#                 new_image = db.session.query(Image).filter(Image.filename==image.filename).filter(Image.camera_id==new_camera_id).first()
#                 if not new_image:
#                     new_image = Image(filename=image.filename, timestamp=image.timestamp, corrected_timestamp=image.corrected_timestamp, camera_id=new_camera_id, hash=image.hash, etag=image.etag, detection_rating=image.detection_rating)
...
#                 # print('Copying detections: {}'.format(len(detections)))
#                 for detection in detections:
#                     new_detection = None
#                     if new_image.id: 
#                         new_detection = db.session.query(Detection).filter(Detection.source==detection.source).filter(Detection.image_id==new_image.id).filter(Detection.left==detection.left).filter(Detection.right==detection.right).filter(Detection.top==detection.top).filter(Detection.bottom==detection.bottom).first()
#                     if not new_detection:
#                         new_detection = Detection(source=detection.source, score=detection.score, left=detection.left, right=detection.right, top=detection.top, bottom=detection.bottom, image=new_image, category=detection.category, status=detection.status, class_score=detection.class_score, classification=detection.classification)
#                         db.session.add(new_detection)

#                     for task_id in task_ids:
#                         new_task_id = new_task_ids[task_ids.index(task_id)]
#                         labelgroups = db.session.query(Labelgroup).filter(Labelgroup.detection_id==detection.id).filter(Labelgroup.task_id==task_id).all()
#                         for labelgroup in labelgroups:
#                             new_labelgroup = None
#                             if new_detection.id:
#                                 new_labelgroup = db.session.query(Labelgroup).filter(Labelgroup.detection_id==new_detection.id).filter(Labelgroup.task_id==new_task_id).first()
#                             if not new_labelgroup:
#                                 new_labelgroup = Labelgroup(detection=new_detection, task_id=new_task_id)
#                                 db.session.add(new_labelgroup)

#                             for label in labelgroup.labels:
#                                 new_label = db.session.query(Label).filter(Label.description==label.description).filter(Label.task_id==new_task_id).first()
#                                 if not new_label:
#                                     new_label = Label(description=label.description, task_id=new_task_id, hotkey=label.hotkey)
#                                     db.session.add(new_label)
#                                 new_labelgroup.labels.append(new_label)
...
#                                     new_tag = Tag(description=tag.description, task_id=new_task_id, hotkey=tag.hotkey)
#                                     db.session.add(new_tag)
#                                 new_labelgroup.tags.append(new_tag)

#                     # db.session.commit()
#                     # detection_map[detection.id] = new_detection.id
...
#                 for task_id in task_ids:
#                     new_task_id = new_task_ids[task_ids.index(task_id)]
#                     clusters = db.session.query(Cluster).filter(Cluster.images.contains(image)).filter(Cluster.task_id==task_id).all()
#                     # print('Copying clusters: {}'.format(len(clusters)))
#                     for cluster in clusters:
#                         new_cluster = db.session.query(Cluster).filter(Cluster.images.contains(new_image)).filter(Cluster.task_id==new_task_id).first()
#                         if not new_cluster:
#                             new_cluster = Cluster(task_id=new_task_id, notes=cluster.notes, classification=cluster.classification)
#                             db.session.add(new_cluster)

#                         new_cluster.images.append(new_image)
...
#                                 new_tag = Tag(description=tag.description, task_id=new_task_id, hotkey=tag.hotkey)
#                                 db.session.add(new_tag)
#                             new_cluster.tags.append(new_tag)

#                         if image in cluster.required_images:
#                             new_cluster.required_images.append(new_image)
...
#             # Copy videos
#             videos = db.session.query(Video).filter(Video.camera_id==camera.id).all()
#             print('Copying videos: {}'.format(len(videos)))
#             for video in videos:
#                 new_video = db.session.query(Video).filter(Video.filename==video.filename).filter(Video.camera_id==new_camera_id).first()
#                 if not new_video:
#                     new_video = Video(filename=video.filename, camera_id=new_camera_id, hash=video.hash)
...
#         # Copy Staticgroups
#         # staticgroups = db.session.query(Staticgroup).join(Detection).join(Image).join(Camera).join(Trapgroup).filter(Trapgroup.id==trapgroup_id).all()
#         # print('Copying staticgroups: {}'.format(len(staticgroups)))
#         # for staticgroup in staticgroups:
#         #     new_staticgroup = Staticgroup(status=staticgroup.status)
#         #     db.session.add(new_staticgroup)

#         #     staticgroup_det_ids = [detection_map[detection.id] for detection in staticgroup.detections]
#         #     staticgroup_dets = db.session.query(Detection).filter(Detection.id.in_(staticgroup_det_ids)).all()
#         #     new_staticgroup.detections.extend(staticgroup_dets)
#         # db.session.commit()
