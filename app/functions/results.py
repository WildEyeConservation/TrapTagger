    ...
    try:
        ...
        for label_level in label_levels:
            if label_type=='column':
                ...
                requestedColumns.remove(label_level+'_sighting_count')
                ...
                if label_level in sighting_count_levels:
                    outputDF[label_level+'_sighting_count'] = outputDF.apply(lambda x: x[label_level+'_'+x[label_level+'_labels'].lower().replace(' ','_')+'_count'], axis=1)
                ...
                if label_level in sighting_count_levels:
                    ...
                    outputDF[label_level+'_sighting_count'] = outputDF.apply(lambda x: combine_list(x[label_level+'_sighting_counts']), axis=1)
...
@celery.task(bind=True,max_retries=29,ignore_result=True)
def prepare_exif(self,task_id,species,species_sorted,flat_structure):
    '''
    ...
    try:
        ...
        if '0' in species:
            ...
            labels.extend(db.session.query(Label).filter(Label.task_id==None).filter(Label.description.in_(['Vehicles/Humans/Livestock'])).distinct().all())
        else:
