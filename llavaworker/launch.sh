...
for ((i=0;$((i<$NUMGPUS));i++)) do
  docker run -e AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY -e REDIS_IP --hostname worker$i@$1 -e WORKER_NAME=$1 -e QUEUE -e WORKER_NUMBER=$i -e CUDA_VISIBLE_DEVICES=$i -v /home/ubuntu/TrapTagger/llavaworker:/code/llavaworker -v /home/ubuntu/TrapTagger/LLaVA/llava:/code/llavaworker/llava --gpus all --ipc=host --name llavaworker$i llava_worker celery -A llavaworker.worker worker -Q $QUEUE -Ofair --concurrency=1 -l info > worker$i.log 2>&1 &
