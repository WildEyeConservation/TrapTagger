...
for ((i=0;$((i<$NUMGPUS));i++)) do
  docker run \
      ...
      -v /home/ubuntu/TrapTagger/gpuworker/init.py:/code/ScarceNet/lib/models/__init__.py \
      -v /home/ubuntu/TrapTagger/wildbook-ia/wbia:/code/wbia \
      --hostname worker$i@$1 \
      --gpus all \
      --ipc=host \
      --name gpuworker$i \
      gpu_worker celery -A gpuworker.worker worker -Q $QUEUE -Ofair --concurrency=1 -l info \
      > worker$i.log 2>&1 &
