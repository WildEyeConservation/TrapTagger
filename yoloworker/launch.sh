...
while $flag; do
  ...
  if [[ "$HTTP_CODE" -eq 401 ]] ; then
    ...
    for ((i=0;$((i<$NUMGPUS));i++)) do
      docker exec yoloworker$i python3 yoloworker/cleanup_worker.py || STATUS=$?
  ...
  if [ $(($(date -u +%s)-$LAUNCH_TIME)) -ge $SETUP_PERIOD ] && [ $((COUNT/$IDLE_MULTIPLIER)) -ge 1 ]; then
    ...
    COUNT=0
    docker exec yoloworker0 bash yoloworker/celery_worker_monitor.sh || STATUS=$?
    echo "STATUS="$STATUS
